...
Environment variables:
  ...
  CATALOG_CACHE      Enable digest-based caching of rendered catalog and
                     bundle data (default: true). Set to false to always
                     render live.
  CATALOG_CACHE_DIR  Directory for all cache files (default: /tmp/catalog-cache).
                     Catalog files: fbc-<package>-<catalog>-<version>-<digest>.json
                     Bundle files:  sha256-<hash>.json
...
check_prereqs() {
  ...
  if [[ "${CATALOG_CACHE}" != "true" || "${CATALOG_CACHE_REFRESH}" == "true" ]]; then
    if command -v skopeo &>/dev/null && ! skopeo login --get-login registry.redhat.io &>/dev/null; then
...
# combination. Uses digest-based caching when enabled; otherwise renders the
# catalog image live with `opm render`.
...
extract_package_data_for_version() {
  ...
  if [[ "${CATALOG_CACHE_REFRESH}" != "true" ]]; then
    local fallback_files=("${CATALOG_CACHE_DIR}/fbc-${PACKAGE_NAME}-${CATALOG}-${version}-sha256_"*.json)
  ...
  local cache_file="${CATALOG_CACHE_DIR}/fbc-${PACKAGE_NAME}-${CATALOG}-${version}-${digest}.json"
  ...
  local old_file
  for old_file in "${CATALOG_CACHE_DIR}/fbc-${PACKAGE_NAME}-${CATALOG}-${version}-sha256_"*.json; do
    [[ -f "${old_file}" ]] || continue
    echo "    Removing stale cache: ${old_file##*/}" >&2
    rm -f "${old_file}"
  done

  opm render "${image}" | jq -c "$jq_filter" > "${cache_file}"
...
# Uses digest-based caching when enabled; auto-populates cache on miss.
...
extract_versions_from_bundle_image() {
  ...
  if [[ "${CATALOG_CACHE}" != "true" || -z "${digest}" ]]; then
    echo "    Rendering bundle ${bundle_image}..." >&2
    bundle_json="$(opm render "${bundle_image}")"
  else
    mkdir -p "${CATALOG_CACHE_DIR}"
    local cache_file="${CATALOG_CACHE_DIR}/sha256-${digest}.json"

    if [[ -f "${cache_file}" ]]; then
      echo "    Cache hit: ${cache_file##*/}" >&2
      bundle_json="$(cat "${cache_file}")"
    elif [[ "${CATALOG_CACHE_REFRESH}" != "true" ]]; then
      echo "    ERROR: No bundle cache for ${digest} and refresh is disabled" >&2
      return 1
    else
      echo "    Cache miss for bundle ${bundle_image##*@}, rendering..." >&2
      bundle_json="$(opm render "${bundle_image}")"
      echo "${bundle_json}" > "${cache_file}"
      echo "      Cached: ${cache_file##*/}" >&2
    fi
  fi
...
main() {
  ...
  echo "Cache: ${CATALOG_CACHE} (refresh: ${CATALOG_CACHE_REFRESH})" >&2
  echo "Mode: ${MODE}" >&2
  echo "" >&2
